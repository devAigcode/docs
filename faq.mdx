---
title: "📕 FAQ"
description: "Frequently asked questions about AutoCoder"
---

## **Get Started**

<AccordionGroup>
  <Accordion title="What is AutoCoder?">
    AutoCoder is the world's first truly end-to-end AI product that automatically generates both the front-end and back-end of applications through AI conversations.
  </Accordion>
  <Accordion title="How is this different from other solutions?">
    - Express your needs through AI conversations, and AutoCoder will generate a detailed requirements list based on your input.
    - Once the project is generated, you can manually edit and adjust it through multiple rounds of AI conversation, with no coding required. Simply use natural language to optimize your project.
    - Completed projects support collaboration and can be easily deployed.
  </Accordion>
  <Accordion title="How can I create a project from the beginning?">
    <Steps>
      <Step title="Step 1">
        Click on the <u>Sign In</u> button to access your AutoCoder account.
      </Step>
      <Step title="Step 2">
        Click on the dialogue box and enter a brief description of the project idea.
      </Step>
      <Step title="Step 3">
        Edit manually or send your request to AutoCoder directly.
      </Step>
      <Step title="Step 4">
        Confirm your requirements and workflow, then start generating.
      </Step>
      <Step title="Step 5">
        Chat with AutoCoder if you want any updates.

        Fantastic! You're now equipped with the essential tools to navigate AutoCoder effortlessly. Keep the conversation flowing and let AI bring your visions to life.
      </Step>
    </Steps>
  </Accordion>
</AccordionGroup>

## **Error & Solution**

<AccordionGroup>
  <Accordion title="What should I do if it takes a little bit longer to generate?">
    You may try **Refresh** or **Regenerate**. If the issue persists, feel free to contact us at [support@autocoder.cc](mailto:support@autocoder.cc)
  </Accordion>
  <Accordion title="What should I do if I want to revert to the previous version?">
    AutoCoder offers version rollback functionality, enabling you to revert to any prior version.
  </Accordion>
  <Accordion title="On which devices is AutoCoder supported?">
    We suggest you can try it on your personal computer with Windows, MacOS, and Linux systems. Alternatively, you can try it on a tablet or smartphone with a browser.
  </Accordion>
  <Accordion title="Which language is supported?">
    Autocoder supports multi-language. You may input your preferred language and will get corresponding reply.
  </Accordion>
</AccordionGroup>

## **Account & Security**

<AccordionGroup>
  <Accordion title="What should I do if I am unable to login?">
    - Make sure your email address is correct and that Caps Lock is off.
    - Ensure you have a stable internet connection. Try clearing your browser cache or switching to a different browser.
    - If the issue persists, feel free to contact us at [support@autocoder.cc](mailto:support@autocoder.cc) with the following details.

    1. Your registered email address
    2. A screenshot of the error message
  </Accordion>
  <Accordion title="What should I do if I want to delete my personal data?">
    - We respect your privacy and offer the option to delete your personal data. Simply follow these steps.
    - Go to the **Account Settings** or **Privacy Settings** page.
    - Select the **Delete My Account** option.
  </Accordion>
  <Accordion title="How do you manage my data?">
    We comply with our Privacy Policy and Terms of Service to ensure that your data is securely handled and protected.
  </Accordion>
</AccordionGroup>

## **Pricing & Payment**

<AccordionGroup>
  <Accordion title="What payment methods are supported?">
    All payments are processed via Stripe. You may use a credit or debit card for payment. Further details are provided on the checkout page.
  </Accordion>
  <Accordion title="Can I continue using my previous plan after upgrading?">
    Upon upgrading, the tokens from your new plan will be prioritized for use. If the upgraded plan's tokens are exhausted, and your previous plan remains active with unused tokens, you may continue to utilize those.
  </Accordion>
  <Accordion title="Must I subscribe to a Sub plan before purchasing a Pro plan?">
    Yes, Pro plans require an active subscription to a Sub plan. We offer enhanced benefits for subscribers, including additional perks when token usage is low.
  </Accordion>
  <Accordion title="Can I downgrade after upgrading?">
    Yes you can. We recommend carefully considering your usage requirements prior to downgrading.
  </Accordion>
  <Accordion title="Do unused credits roll over?">
    Monthly credits expire at the end of each billing cycle and do not roll over. Additional credit packages are available for your choice. This policy encourages continuous development while providing flexibility for larger projects.
  </Accordion>
</AccordionGroup>

## **Export Project**

<AccordionGroup>
  <Accordion title="What is the quality of the exported code? Can I run it immediately?">
    Yes. The export includes a complete project structure with all necessary environment configurations. Simply follow the included `README.md` and run `npm install` to get your project running locally in minutes.
  </Accordion>
  <Accordion title="Can I re-import the code back into the platform after editing it locally?">
    Currently, we only support one-way export. You can download the source code for local development or hosting, but we do not support importing manual code changes back into the platform at this time.
  </Accordion>
  <Accordion title="Can I sync my project directly to GitHub?">
    Currently, projects are exported as a ZIP file which you can manually push to GitHub. Native GitHub integration is under active development—stay tuned for future updates.
  </Accordion>
  <Accordion title="Does the export only include the frontend? What about Web and App projects?">
    The export is a full-stack package including the **frontend, backend, and database schemas**. It supports both Web and App projects. You will just need a local IDE and the appropriate database environment to run it.
  </Accordion>
  <Accordion title="Are there any restrictions on the exported code or its features?">
    Code Export is available to our **Standard and Pro** monthly subscribers. Once subscribed, there are no limits on the number of times you can export your project code.
  </Accordion>
</AccordionGroup>

## **Refund Policy**

<AccordionGroup>
  <Accordion title="Refund Eligibility">
    Refunds are available under the following conditions:

    - The service must be purchased directly through our website.
    - Refund requests must be submitted within 7 days of the service purchase date.
  </Accordion>
  <Accordion title="Non-Refundable Services">
    No refunds are available for services that have already been used.
  </Accordion>
  <Accordion title="Refund Amount">
    Refunds may incur a processing fee, depending on the applicable charge standards.
  </Accordion>
  <Accordion title="Refund Delays or Issues">
    - Refunds will be issued to the original payment method.
    - For expired or canceled cards, the refund will be processed by your card issuer. If you haven't received your refund, please check your bank account, then contact your credit card company or bank. Refunds may take some time to process. If you've done this and still haven't received your refund, please contact us at [support@autocoder.cc](mailto:support@autocoder.cc).
  </Accordion>
  <Accordion title="Changes to This Policy">
    We reserve the right to update or modify this refund and return policy at any time. Any changes will be posted on this page and will take effect immediately.
  </Accordion>
  <Accordion title="Contact Us">
    If you have any questions about our refund policy, please contact us via email at [support@autocoder.cc](mailto:support@autocoder.cc).
  </Accordion>
</AccordionGroup>